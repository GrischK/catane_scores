...
import {
  useCreateGameWithScoresMutation,
  User,
  useUsersByIdsQuery,
  useUsersQuery,
...
export default function NewGame({ refreshGamesList }: any) {
  ...
  const onClickCreateNewGame = async () => {
    const isGameNotFilledWithPlayers =
      newGame.playersData.length < 2 || newGame.playersData.length > 6;
    ...
    const areAllScoresFilled = newGame.playersData.every(
    ...
    <div className={styles.new_game_container}>
      ...
      >
        <div className={styles.input_container}>
          ...
          >
            <div ref={containerRef} className={styles.players_list_container}>
              <label htmlFor="playersList" onClick={handleModal}>
                <motion.div
              ...
              gamePlayers.map((e) => (
                <div className={styles.new_game_players_item} key={e.id}>
                  ...
                  <ColoredInput
                    ...
                    label={"score"}
                    value={
                      newGame.playersData.find(
                        (player) => player.player === e.id,
                      )?.score || ""
                    }
                    onChange={(event) => {
        ...
        <motion.div
          ...
          transition={buttonTransition}
        >
          <ColoredButton bgColor={"red"} onClick={onClickCreateNewGame}>
            Créer le Catanage
          </ColoredButton>
        </motion.div>
      </div>
      ...
      >
        <Box
          ...
          className={styles.add_players_modal_box}
        >
          <Typography id="modal-modal-title" variant="h6" component="h2">
            Choisis les Cataneurs qui s'affrontent :
          </Typography>
          <div className={styles.modal_players_container}>
            {userNames.map((user) => (
              <div key={user.id} className={styles.game_player_wrapper}>
                <label
                ...
                >
                  ...
                  <input
                    disabled={
                      newGame.playersData.length >= 6 &&
                      !newGame.playersData.some(
                    ...
                    value={user.name}
                    checked={newGame.playersData.some(
                      (player) => player.player === user.id,
                    )}
                    onChange={(event) => {
                        ...
                        playersData: isChecked
                          ? [
                              ...prevState.playersData,
                              {
                                player: user.id,
                                score:
                                  prevState.playersData.find(
                                    (player) => player.player === user.id,
                                  )?.score || 0,
                              },
                            ]
                          : prevState.playersData.filter(
                              (player) => player.player !== user.id,
                            ),
                      }));
                    }}
          ...
          <motion.div
            ...
            transition={buttonTransition}
          >
            <ColoredButton bgColor={"red"} onClick={handleCloseModal}>
              OK
            </ColoredButton>
          </motion.div>
          <div className={styles.add_players_close_modal_icon}>
